...
// Add Authorization token for tls handshake
func (q *QUICConn) SetAuthorizationToken(token string) {
	q.conn.addAuthToken(token)
}

// Add Authorization function for tls handshake
func (q *QUICConn) SetAuthorizationFn(fn func(string) bool) {
	q.conn.authFn = fn
}
